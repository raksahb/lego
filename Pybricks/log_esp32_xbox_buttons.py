...
Requirements:
- LEGO Hub running Pybricks firmware.
- LMS-ESP32 flashed with BluePad firmware, connected to the hub 
  (e.g. port 'F').
- A supported gamepad (e.g. XBox Controller).

This script initializes the BluePad module, reads the gamepad state, 
decodes button presses and D-pad directions, 
and displays corresponding icons on the LEGO Hub's display. 
It also logs the gamepad input to the console.
"""

from bluepad import bluepad_init, get_buttons, get_direction_pad
